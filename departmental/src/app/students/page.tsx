...
const StudentsPage = () => {
  ...
  const levelCollection = createListCollection({
    items: levels.map((lvl) => ({
      label: lvl === "all" ? "All Levels" : `${lvl} Level`,
      value: lvl,
    })),
  });

  const programCollection = createListCollection({
    items: [{ label: "All Programs", value: "all" }],
  });

  const sessionCollection = createListCollection({
    items: [{ label: "All Sessions", value: "all" }],
  });
    ...
    <Box>
      ...
      <Flex direction={{ base: "column", md: "row" }} justifyContent="space-between" alignItems={{ base: "flex-start", md: "center" }} mb="10" gap="4">
        ...
        <Flex alignItems="center" gap="3" flexWrap="wrap">
          <Button onClick={handleDownloadTemplate} display="flex" alignItems="center" gap="2" px="4" py="2.5" bg="white" border="1px solid" borderColor="border.subtle" color="#1D7AD9" borderRadius="lg" fontSize="sm" fontWeight="bold" cursor="pointer">
            <FileDown size={18} /> Download Sample File
          </Button>
          <Button onClick={() => setShowUpload(true)} display="flex" alignItems="center" gap="2" px="4" py="2.5" bg="white" border="1px solid" borderColor="border.subtle" color="#1D7AD9" borderRadius="lg" fontSize="sm" fontWeight="bold" cursor="pointer">
            <FileUp size={18} /> Upload CSV
          </Button>
          <Button onClick={() => { setStudentToEdit(null); setShowAddForm(true); }} bg="#1D7AD9" color="white" px="6" py="2.5" borderRadius="lg" display="flex" alignItems="center" gap="2" fontSize="sm" fontWeight="bold" boxShadow="lg" cursor="pointer">
        ...
        <Flex alignItems="center" justifyContent="space-between" gap="4" flexWrap="wrap">
          ...
          <Flex alignItems="center" gap="3">
            ...
            <Select.Root
              ...
              onValueChange={(e) => setSelectedProgram(e.value[0])}
              size="md"
              width="150px"
            >
              <Select.HiddenSelect />
              <Select.Control>
                <Select.Trigger>
                  <Select.ValueText placeholder="All Programs" />
                </Select.Trigger>
                <Select.IndicatorGroup>
                  <Select.Indicator />
                </Select.IndicatorGroup>
              ...
              <Portal>
                <Select.Positioner>
                  <Select.Content>
                    <Select.Item item={{ label: "All Programs", value: "all" }}>All Programs</Select.Item>
                  </Select.Content>
                </Select.Positioner>
              </Portal>
            </Select.Root>
            ...
            >
              ...
              <Portal>
                <Select.Positioner>
                  <Select.Content>
                    <Select.Item item={{ label: "All Sessions", value: "all" }}>All Sessions</Select.Item>
                  </Select.Content>
                </Select.Positioner>
              </Portal>
            </Select.Root>

            <Button onClick={clearFilters} display="flex" alignItems="center" gap="2" px="6" py="2" bg="white" border="1px solid" borderColor="slate.200" borderRadius="lg" fontSize="xs" fontWeight="semibold" color="slate.800" cursor="pointer">
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="4">
        ...
        <Button onClick={handleExport} display="flex" alignItems="center" gap="2" px="4" py="2" bg="white" border="1px solid" borderColor="slate.200" borderRadius="xl" fontSize="xs" fontWeight="semibold" color="slate.600" cursor="pointer" >
      ...
      {loading ? (
        ...
        <Box textAlign="center" py="16" bg="white" borderRadius="2xl" border="1px solid" borderColor="border.subtle">
        ...
        <Box bg="white" borderRadius="2xl" border="1px solid" borderColor="border.subtle" boxShadow="sm" overflow="hidden" maxW={{ base: "100%", lg: "calc(100vw - 340px)" }}>
          <Box overflowX="auto">
            <Box as="table" w="full" textAlign="left">
              <Box as="thead">
                <Box as="tr" bg="slate.50" borderBottom="1px solid" borderColor="border.subtle" fontSize="11px" fontWeight="bold" color="slate.500" textTransform="uppercase" letterSpacing="wider" whiteSpace="nowrap">
              ...
              <Box as="tbody" fontSize="xs">
                {paginatedStudents.map((s) => (
                  <Box as="tr" key={s.id} _hover={{ bg: "slate.50" }} borderBottom="1px solid" borderColor="border.subtle" bg={selectedIds.includes(s.id) ? "blue.50" : undefined} cursor="pointer" whiteSpace="nowrap">
                    <Box as="td" px="6" py="5" textAlign="center" position="sticky" left="0" zIndex="10" bg={selectedIds.includes(s.id) ? "blue.50" : "white"} borderBottom="1px solid" borderColor="border.subtle">
                    ...
                    <Box as="td" px="6" py="5" textAlign="right" pr="12" position="sticky" right="0" zIndex={activeDropdownId === s.id ? "50" : "10"} bg={selectedIds.includes(s.id) ? "blue.50" : "white"} borderBottom="1px solid" borderColor="border.subtle" ref={dropdownRef}>
                      <Box position="relative">
                        <Button onClick={(e: React.MouseEvent) => toggleDropdown(s.id, e)} p="1" _hover={{ bg: "slate.100" }} borderRadius="full" cursor="pointer" border="none" bg="transparent" color="slate.400">
                        ...
                        {activeDropdownId === s.id && (
                          <Box position="absolute" right="0" top="8" w="48" bg="white" borderRadius="xl" boxShadow="xl" border="1px solid" borderColor="gray.100" zIndex="50" overflow="hidden" textAlign="left">
                            <Box p="1">
                              <Button onClick={(e: React.MouseEvent) => { e.stopPropagation(); setSelectedStudent(s); setActiveDropdownId(null); }} w="full" display="flex" alignItems="center" gap="2" px="3" py="2" fontSize="sm" fontWeight="medium" color="green.600" _hover={{ bg: "green.50" }} borderRadius="lg" cursor="pointer" border="none" bg="transparent">
                                <UserCog size={16} /> Assign Role
                              </Button>
                              <Button onClick={(e: React.MouseEvent) => { e.stopPropagation(); setStudentToEdit(s); setShowAddForm(true); setActiveDropdownId(null); }} w="full" display="flex" alignItems="center" gap="2" px="3" py="2" fontSize="sm" fontWeight="medium" color="amber.600" _hover={{ bg: "amber.50" }} borderRadius="lg" cursor="pointer" border="none" bg="transparent">
        ...
        <Flex alignItems="center" justifyContent="space-between" bg="white" borderRadius="2xl" border="1px solid" borderColor="gray.100" boxShadow="sm" p="4" mt="4">
          ...
          <Flex alignItems="center" gap="2">
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <Button key={pageNum} onClick={() => setCurrentPage(pageNum)} px="3" py="2" borderRadius="lg" fontSize="sm" fontWeight="medium" cursor="pointer" border={currentPage === pageNum ? "none" : "1px solid"} borderColor="slate.200" bg={currentPage === pageNum ? "#1D7AD9" : "white"} color={currentPage === pageNum ? "white" : "slate.700"} _hover={{ bg: currentPage === pageNum ? "#1D7AD9" : "slate.50" }}>
                  {pageNum}
                </Button>
              );
            })}
      ...
      {selectedIds.length > 1 && (
        <Flex position="fixed" bottom="8" left="50%" transform="translateX(-50%)" bg="white" px={{ base: "4", md: "6" }} py="3" borderRadius="xl" boxShadow="2xl" border="1px solid" borderColor="gray.100" alignItems="center" gap={{ base: "3", md: "6" }} zIndex="50" flexWrap="wrap" justifyContent="center" w={{ base: "90%", md: "auto" }}>
          ...
          <Button onClick={handleBulkDownload} display="flex" alignItems="center" gap="2" bg="#1D7AD9" color="white" px="4" py="2" borderRadius="lg" fontSize="xs" fontWeight="bold" _hover={{ bg: "blue.700" }} cursor="pointer" border="none">
